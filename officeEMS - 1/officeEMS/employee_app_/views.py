...
#from .models import Employees, Roles, Departments
...
def add_employees_(request):
    if request.method == 'POST':
        ...
        new_emp.save()
        return HttpResponse('''<b> Employees added Successfully..... | 
            <a href='/'>HOME</a></b>''')
...
def remove_employees_(request, emp_id = 0):
    if emp_id:
        ...
        return HttpResponse('''<b> Employees Removed Successfully..... | 
            <a href='/'>HOME</a></b>''')
    emps = Employees.objects.all()
    context = {
        'emps': emps
...
def filter_employees_(request):
    if request.method == 'POST':
        ...
        if location:
            # remember that, with that below logic we return list, not QuerySet...
            locations = Departments.objects.filter(location=location)
            locationIDs = [ q.id for q in locations]
            emps_=emps
            emps=list()
            for emp_ in emps_:
                if(emp_.dept_id in locationIDs):
                    emps.append(emp_)
        data=list()
        mno=1 
        for emp in emps:
    ...
    role = Roles.objects.all()
    dept = Departments.objects.all()
    location = { q.location for q in dept }
    context = {
        'roles' : role,
        'depts' : dept,
        'locations' : location,
...
def update_employees_(request, emp_id = 0):
    ...
    if request.method == 'POST':
        ...
        lock.save()
        return HttpResponse('''<b> Employees updated Successfully..... | 
            <a href='/'>HOME</a></b>''')
